...
class LDPCNetwork(nn.Module):
    ...
    def _init_weight(self, s_val, prefix):
        ...
        if prefix in ["cn", "ucn"]:
            if s_val == 1:
                ...
                w = torch.ones(self.M_proto, device=self.device) * self.init_cn_weight  # Custom per-feature
            elif s_val == 5:
                w = torch.ones(1, device=self.device) * self.init_cn_weight  # Global scalar
            else:
                return None
        elif prefix == "ch":
            if s_val == 2:
                ...
                w = torch.ones(self.N_proto, device=self.device) * self.init_ch_weight  # Custom per-feature
            elif s_val == 5:
                w = torch.ones(1, device=self.device) * self.init_ch_weight  # Global scalar
            else:
                return None
        else:
            return None
    ...
    def _apply_ch_weight(self, llr_in, it_idx):
        ...
        if s_val == 4:  # Custom per-feature weights
            # Map N_proto to N by repeating weights across the expansion
            v = torch.arange(self.N, device=self.device)
            proto_col = torch.div(v, self.N // self.N_proto, rounding_mode='trunc')
            w_iter = self.ch_weight[proto_col]
            w_iter = w_iter.unsqueeze(0).expand(llr_in.size(0), -1)
            return llr_in * w_iter
        elif s_val == 5:  # Global scalar weight
            scalar = self.ch_weight
    ...
    def _apply_cn_weight(self, c2v_llr, it_idx):
        ...
        if s_val == 1:
            ...
            proto_row = torch.div(self.edge_to_cn, self.z_factor, rounding_mode='trunc')
    ...
    def _cn_update_MS_par(self, v2c_llr):
        ...
        2) Compute min across dim=2, product of signs across dim=2.
        ...
        # Ensure ext_idx is of type torch.long
        ext_idx = ext_idx.to(torch.long)
