...
def apply_quantization(llrs, q_bit):
    return Cal_MSA_Q(llrs, q_bit)
...
def run_validation(args, net, code_param, device, rng, epoch):
    ...
    if args.sampling_type == 0:
        ...
        llrs_valid = apply_quantization(llrs_valid, args.q_bit)
    else:
...
def run_training(args, net, code_param, device, rng, epoch):
    ...
    if args.sampling_type == 0:
        ...
        llrs_train = apply_quantization(llrs_train, args.q_bit)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--decoding_type', type=int, default=1) #0: SP, 1: MS, 2:QMS
    parser.add_argument('--q_bit', type=int, default=5) # Quantization bits for QMS
    ...
    parser.add_argument('--loss_option', type=int, default=0) #0: BCE, 1: Soft_BER, 2: FER
    ...
    parser.add_argument('--training_num', type=int, default=30000)
